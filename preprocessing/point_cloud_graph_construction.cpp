...
void PointCloudGraphConstructor::lEsfNodeFeatures(double** result) {
  ...
  std::vector< float > k_sqr_distances;
  Eigen::Vector4f v1, v2, n1, n2, v12;
  double dist, a12, a12n1, a12n2, occ_r;
  double feat_min = -0.5;
  double feat_max =  0.5;
  double max_dist = 0.;
    ...
    for (uint index1=0; index1 < k_indices.size(); index1++) {
      for (uint index2=index1+1; index2 < k_indices.size(); index2++) {
        ...
        if (std::isnan(dist) || std::isnan(a12) || std::isnan(a12n1) ||
            std::isnan(a12n2) || std::isnan(occ_r))
          continue;


        // Fill in the matrix
        result[pt_idx][5*pair_nb + 0] = dist;
        result[pt_idx][5*pair_nb + 1] = occ_r;
        result[pt_idx][5*pair_nb + 2] = a12;
        result[pt_idx][5*pair_nb + 3] = a12n1;
        result[pt_idx][5*pair_nb + 4] = a12n2;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void PointCloudGraphConstructor::viz() {
